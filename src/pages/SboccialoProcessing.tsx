...
import { StageBatchInput, ScrapDetails } from '../types';
import { supabase } from '../lib/supabase';

export const SboccialoProcessing: React.FC = () => {
  const { lots, addStageBatch, recordMovement } = useContext(GestionaleContext);
  ...
  const validateAndSave = async () => {
    ...
    // Validate: input = output + scrap + residual (±2% tolerance)
    ...
    if (diff > tolerance) {
      alert(
        `⚠️ Weight mismatch!\nInput: ${input}kg\nExpected: ${output}+${scrap}+${residual}=${expected}kg\nDiff: ${diff}kg (tolerance: ${tolerance}kg)`
      );
      return;
    }

    // Validate scrap details sum
    ...
    if (Math.abs(scrapSum - scrap) > 0.1) {
      alert(`⚠️ Scrap details don't match total scrap!\nBiomass+Mould+Wood: ${scrapSum}kg\nTotal Scrap: ${scrap}kg`);
      return;
    }
    ...
    if (Math.abs(distSum - output) > 0.1) {
      alert(`⚠️ Category distribution doesn't match output!\nTotal: ${distSum}kg\nExpected: ${output}kg`);
      return;
    }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Sala Sboccialo (Primo Stadio)</h2>

      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium mb-1">Lot Code *</label>
          <input
            type="text"
            value={form.lotCode}
      ...
      <div className="bg-gray-800 p-4 rounded mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => handleFormChange('inputWeight', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => handleFormChange('outputWeight', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Total Scrap *</label>
            <input
              type="number"
              step="0.001"
              value={form.scrapWeight}
              onChange={(e) => handleFormChange('scrapWeight', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Residual *</label>
            <input
              type="number"
              step="0.001"
              value={form.residualWeight}
              onChange={(e) => handleFormChange('residualWeight', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={form.scrapBiomass}
              onChange={(e) => handleFormChange('scrapBiomass', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={form.scrapMould}
              onChange={(e) => handleFormChange('scrapMould', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={form.scrapWood}
              onChange={(e) => handleFormChange('scrapWood', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={distribution.grandi}
              onChange={(e) => handleDistributionChange('grandi', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={distribution.medi}
              onChange={(e) => handleDistributionChange('medi', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={distribution.mini}
              onChange={(e) => handleDistributionChange('mini', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="0.001"
              value={distribution.trinciato}
              onChange={(e) => handleDistributionChange('trinciato', e.target.value)}
              className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
              placeholder="0.000"
            />
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleFormChange('operatorEmail', e.target.value)}
            className="w-full px-3 py-2 border rounded bg-gray-900 text-white"
